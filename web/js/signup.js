...
function showPrivacyPopup() {
    showLegalPopup('개인정보 수집 및 이용 동의', ''
        ...
        + '1. 회원 식별 및 가입 승인<br>'
        + '2. 커뮤니티 서비스 제공 (게시판, 일정, 소모임, 회의록)<br>'
        + '3. 공지사항 및 푸시 알림 발송<br>'
        + '4. 회원 간 연락처 공유 (앱 내 한정, 회원이 설정한 공개 범위에 따름)<br>'
        + '5. JC 지도 서비스 제공 (사업장 위치 표시)<br>'
        + '6. 회원 프로필 홍보 (배너 노출, 사업 PR)</p>'
        + '<p><strong>3. 개인정보 보유 및 이용 기간</strong><br>'
        + '회원 탈퇴 시부터 90일간 보관 후 완전 파기합니다.<br>'
        + '삭제된 게시글·일정은 90일 후 자동 완전 파기됩니다.<br>'
        + '관련 법령에 따라 보존이 필요한 경우 해당 기간 동안 보관합니다.</p>'
        + '<p><strong>4. 개인정보의 제3자 제공</strong><br>'
        + 'JC는 원칙적으로 개인정보를 외부에 제공하지 않습니다. 다만 다음의 경우 예외로 합니다.<br>'
        + '- 회원이 사전에 동의한 경우 / 법률에 특별한 규정이 있는 경우<br>'
        + '- 타 로컬 회원 검색 시: 이름, 회사, 업종, 프로필 사진이 공개 범위 내에서 노출<br>'
        + '- 소모임 참여 시: 멤버 간 이름, 회사, 소속 조직 정보 공유</p>'
        + '<p><strong>5. 개인정보 처리 위탁</strong><br>'
        + '주소 좌표 변환: Kakao (카카오 주소 API) — 입력된 주소를 좌표로 변환하기 위해 전달됩니다.</p>'
        + '<p><strong>6. 이용자의 권리</strong><br>'
        + '이용자는 언제든지 본인의 개인정보를 열람, 정정, 삭제 요구할 수 있으며, 프로필 수정 또는 회원 탈퇴를 통해 처리할 수 있습니다.<br>'
        + '- 연락처 공개 범위 설정 변경 (프로필 > 전화번호 공개 범위)<br>'
        + '- 푸시 알림 수신 거부 (설정 > 알림 설정)<br>'
        + '- JC 지도 노출 여부 설정 (프로필 수정 > 지도 노출)</p>'
        + '<p><strong>7. 개인정보 보호 책임자</strong><br>'
        + '개인정보 보호책임자: 김우연<br>문의: 앱 내 설정 > 문의하기</p>'
        + '<p style="color:var(--text-hint);font-size:13px;margin-top:16px">시행일: 2026년 3월 31일</p>'
    );
}
